...
router.route("/:id").get(verifyRoles(ROLES_LIST.User), usersController.getUser);
...
router
  ...
  .get(
    ...
    coursesController.getAllUserCourses
  );

router
  .route("/teacher/:id/courses")
  .get(
    verifyRoles(ROLES_LIST.Teacher, ROLES_LIST.User),
    coursesController.getAllTeacherCourses
  );
